...
cvar_t m_accelerate_natural_strength = {CVAR_SAVE, "m_accelerate_natural_strength", "0", "How quickly the accelsensitivity approaches the m_accelerate_natural_accelsenscap, values are compressed between 0 and 1 but higher numbers are allowed"};
cvar_t m_accelerate_natural_accelsenscap = {CVAR_SAVE, "m_accelerate_natural_accelsenscap", "0", "Horizontal asymptote that sets the maximum value for the natural mouse acceleration curvel, value 2, for example, means that the maximum sensitivity is 2 times the base sensitivity"};
cvar_t m_accelerate_natural_offset = {CVAR_SAVE, "m_accelerate_natural_offset", "0", "below this speed in px/ms, no natural acceleration is done"};
	...
	if(m_accelerate.value > 0)
	{
		...
		if (m_accelerate_natural_strength.value > 0.0f && m_accelerate_natural_accelsenscap.value >= 0.0f)
		{
			...
			if (adjusted_speed_pxms > 0 && m_accelerate_natural_accelsenscap.value != 1.0f)
			{
				...
				accelsens += (adjusted_accelsenscap - adjusted_accelsenscap * exp( - ((adjusted_speed_pxms * m_accelerate_natural_strength.value) / fabs(adjusted_accelsenscap) ))) * inv_sensitivity;
			}
